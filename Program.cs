...
`------' `--' `----'`--''--'  `--'   `----'`--''--'`--' `---'  `---'      `--`--'    `------'  `--`--'`--' `---'  `----' `--'   `----'     `--' '--' `--`--' `---' `----' 
        ...
        if (opcion == ConsoleKey.Q)
        {
            ...
            if (opcion2 == ConsoleKey.Q)
            {
                ...
                Console.WriteLine();
                Console.WriteLine("🧙:Rollo,un mago extremadamente poderoso y extremadamente senil,te siguio hasta la tumba pidiendo comida y la proxima vez que te vea te contara una de sus batallitas,te aturde durante un turno y te envia a tu posicion inicial");
                Console.WriteLine("👩:Tu Mama,tu querida mami que se entero que andabas enfermito y fue al laberinto a cuidarte jovenzuelo,triplica tu movimiento y quita el cooldown de tu habilidad");
                Console.WriteLine("🔪:Pedrito Heroe de la Confronta,un heroe procedente de El Aplanado,con cierta historia criminal,reducira tu velocidad a la mitad durante un turno");
